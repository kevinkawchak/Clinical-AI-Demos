...
\midrule
On-premises repository based LLMs provide commands to humanoids & llm planner, broadcaster, comms intellectual. \\
...
Workflow minimizes robot error & swarm coordinator (191 lines, camaraderie factor of 3). \\
\bottomrule
\end{tabular}
\end{table}
...
\fp{paper/codegen/src/prompt_frozen.md} pins the planner role
...
or the no fly zones at the patient head. 

This contract has a direct
regulatory consequence: an FDA reviewer or an IRB member can audit
the safety envelope from the constants in \fp{xyz_safety.py} alone
...
\begin{figure}[H]
    \hspace{0.1cm}
   \includegraphics[width=1\linewidth, trim={0 0 0 0}, clip]{Images/07-02-operational-timeline-gantt.png}
    % \vspace{-0.25cm}
    \caption{Per site and per rotation weekly gantt for three Unitree H2 robots}
    \label{Gantt}
\end{figure}
...
\fp{paper/codegen/src/fda_rtct_submitter.py} signs every payload
...
\cite{fda_rtct_2026, fda2026realtime} seen in \autoref{Gantt}. The CTCAE grader at
\fp{paper/codegen/src/ctcae_grader.py} applies grading
across the 13 adverse event types in the decision table
\cite{ctcae_v5_2017}. The error checker at
\fp{paper/codegen/src/check_errors.py} traps schema and physics rule
...
regulatory review (\autoref{Matrix})work as a task that the project has already
...
\begin{figure}[H]
    % \hspace{-0.75cm}
   \includegraphics[width=1\linewidth, trim={0 0 0 0}, clip]{Images/07-03-regulatory-compliance-heatmap.png}
    % \vspace{-0.25cm}
    \caption{Compliance matrix for triple robot workflow using Claude Code.}
    \label{Matrix}
\end{figure}
...
\begin{figure}[H]
    \hspace{2.75cm}
   \includegraphics[width=0.7\linewidth, trim={0 0 0 0}, clip]{Images/07-09-strategic-quadrant-matrix.png}
    % \vspace{-0.25cm}
    \caption{Strategic quadrant diagram of humanoids version humans per site.}
    \label{Quad}
\end{figure}
...
coordination required to enroll the first real PAT-NET-001 cohort shown in \autoref{Quad}.
