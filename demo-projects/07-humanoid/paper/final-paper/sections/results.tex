...
identical across the 4 sites: this parity reflects the 1 Hz broadcast cadence and the shared decision schema, not duplicated content. AT-01 Lead Hz at scale file. }
...
\midrule
SF-01 (San Fran.) & SF-01 llm decisions jsonl & 62{,}330 & 60 decisions captured. \\
...
AT-01 (Atlanta) & AT-01 llm decisions jsonl & 62{,}330 & 60 decisions captured. \\
\bottomrule
\end{tabular}
\end{table}
...
\ref{tbl:iteration-summary} extracts headline numbers from the
codegen index.

% \begin{figure}[H]
% \hspace{1.75cm}
%    \includegraphics[width=0.8\linewidth, trim={0 0 0 0}, clip]{Images/02_response_time_histogram.png}
%     % \vspace{-0.25cm}
%     \caption{New Caption}
%     \label{FourSumm}
% \end{figure}
...
Adverse event/iteration & 84 & 22 G1, 38 G2, 18 G3, 5 G4, 1 G5. \\
...
Camaraderie invariants pass & 0.954 to 0.985 & 7 invariants enforced by coordinator. \\
FDA RTCT 1 hour compliance & 0.9986 to 0.999 & \raisebox{0.25ex}{\scalebox{0.7}{$\geq$}}Grade 2 submission in HR 9505 SLA. \\
Swarm uptime (\%) & 99.66 to 99.70 & Tick coverage across 168 hour window. \\
\bottomrule
\end{tabular}
\end{table}
...
3 human paramedic team & 0.682 & 375 (misses 90 s SLA, 6 reference studies) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
    % \hspace{-0.75cm}
   \includegraphics[width=1\linewidth, trim={0 0 0 0}, clip]{Images/fig-07-comparison-radar.png}
    % \vspace{-0.25cm}
    \caption{Head-to-head four robot metrics as a function of weighted score rankings.}
    \label{Radar2}
\end{figure}
...
overlap on the radar. The v0.4.0 camarade shown in \autoref{Radar2} swarm dominates on
...
\midrule
Sites & 4 & \fp{instructions/README.md} \\
...
Per arm force, compressions & 15 N & \fp{codegen/src/xyz_safety.py} \\
Per arm force otherwise & 5 N & \fp{codegen/src/xyz_safety.py} \\
Inter robot distance at rest & 1.2 m & \fp{codegen/src/xyz_safety.py} \\
Inter robot distance, handoff & 0.4 m & \fp{codegen/src/xyz_safety.py} \\
Cross robot force, transfer & 22 N & \fp{codegen/src/xyz_safety.py} \\
...
Metrics & R7 & 7 & instructions & Head. table & 1 page & Sourced & Done \\
}
